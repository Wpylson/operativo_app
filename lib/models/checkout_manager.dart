...
class CheckoutManager extends ChangeNotifier{
  ...
  Future<int> _getOrderId()async{
    final ref = firestore.document('aux/ordercounter');//pegar o doc no firestores
   try{
    final result = await firestore.runTransaction((tx)async{//Transacao
        final doc = await tx.get(ref);//ler doc do firestore
        final orderId = doc.data['current'] as int;//atribuir na var orderId o valor actual da bd
        await tx.update(ref, {'current': orderId + 1});//pegar o valor e actualizar com +1 para torna-lo sempre unico
  ...
  Future<void> _decrementStock(){
    ...
    return firestore.runTransaction((tx) async{
      ...
      for(final cartProduct  in cartManager.items){//permite acessar cada um dos cartProduto
        ...
        if(productsToUpdate.any((p) => p.id == cartProduct.productId)){
          ...
          final doc = await tx.get(
              firestore.document('produtos/${cartProduct.productId}')//stock actual do produto
          );
