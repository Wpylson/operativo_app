...
class HomeManager extends ChangeNotifier{
  ...
  Future<void> _loadSections()async{
    firestore.collection('home').orderBy('pos').snapshots().listen((snapshot) {
      _sections.clear();//limpar os dados anteriores e trazer os dados actualizados do firebase
      for(final DocumentSnapshot document in snapshot.documents){
        _sections.add(Section.fromDocument(document));//Add na List<Section> o dados do firebase para popular no Objecto Section
      }
      notifyListeners();
    });
  }
